...
class cComponent extends cDefaultList
{
    ...
    cPar par[] @getter(par) @sizeGetter(getNumParams) @label(parameters) @group(parameters, gates) @hint("NED parameters");
    cProperty *properties_[] @label(properties) @getter(->getProperties()->get(i)) @sizeGetter(->getProperties()->getNumProperties()) @group(parameters, gates) @hint("NED properties");
    cComponent::SignalListenerList listenedSignals[] @getter(getListenerList) @sizeGetter(getSignalTableSize) @toString(.str()) @group(signals, statistics) @hint("Signal listener lists");
    cResultRecorder *results[] @getter(->getResultRecorders()[i]) @sizeGetter(->getResultRecorders().size()) @toString(->str()) @group(signals, statistics) @hint("Statistical results collected in this component");
}
...
class cStatistic extends cOwnedObject
{
    ...
    @descriptor(readonly);
    @icon(statistic);
    long count @group(statistical summary) @hint("Number of observations collected, regardless of their weights");
    double sumWeights @group(statistical summary) @getter(getWeights) @hint("Sum of weights, which equals the number of observations in the unweighted case");
    double min @group(statistical summary) @hint("Minimum of the observations");
    double max @group(statistical summary) @hint("Maximum of the observations");
    double mean @group(statistical summary) @hint("Mean of the observations");
    double stddev @group(statistical summary) @hint("Standard deviation of the observations");
    double variance @group(statistical summary) @hint("Variance of the observations");
}
...
class cAbstractHistogram extends cStdDev
{
    ...
    @overwritePreviousDefinition;
    @descriptor(readonly);
    bool binsAlreadySetUp @getter(binsAlreadySetUp) @group(histogram) @hint("If false, the object is still collecting initial observations to gather information for laying out histogram bins");
    double underflowSumWeights  @group(histogram) @hint("Total weight of observations above the last histogram bin");
    double overflowSumWeights @group(histogram) @hint("Total weight of observations above the last histogram bin");
    int numBins @group(histogram) @hint("Number of bins in the histogram");
    cAbstractHistogram::Bin binInfo[] @getter(internalGetBinInfo) @sizeGetter(getNumBins) @group(histogram) @hint("Histogram bin bounds, observation count in the bin, and density estimate in the bin");
}
...
class cLegacyHistogramBase extends cPrecollectionBasedDensityEst
{
    @existingClass;
    @overwritePreviousDefinition;
    @descriptor(readonly);
...
class cLegacyHistogram extends cLegacyHistogramBase
{
    @existingClass;
    @overwritePreviousDefinition;
    @descriptor(readonly);
...
class cVarHistogram extends cLegacyHistogramBase
{
    @existingClass;
    @overwritePreviousDefinition;
    @descriptor(readonly);
...
class cLongHistogram extends cLegacyHistogram
{
    @existingClass;
    @overwritePreviousDefinition;
    @descriptor(readonly);
...
class cDoubleHistogram extends cLegacyHistogram
{
    @existingClass;
    @overwritePreviousDefinition;
    @descriptor(readonly);
...
class cPolylineFigure extends cAbstractLineFigure
{
    ...
    @overwritePreviousDefinition;
    @descriptor(readonly);
    Point points[] @group(geometry) @editable @getter(getPoint) @setter(setPoint) @sizeGetter(getNumPoints);
